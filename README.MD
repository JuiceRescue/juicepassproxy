...
| **CRON_REBOOT_SCHEDULE**  | No                                                            | Default: false. If set, the JuiceBox will automatically reboot at the specified cron schedule. This can be useful for ensuring the JuiceBox remains stable over long periods of time. The format follows standard cron syntax. For example, to reboot every day at midnight, set `CRON_REBOOT_SCHEDULE` to `0 0 * * *`. |
